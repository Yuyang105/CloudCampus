...
$con = mysql_connect("tcss545-aws-db.ce2yjmz6wbyv.us-west-2.rds.amazonaws.com","Yuyang","3350335012");
					...
					if($_SESSION[mail]!=""){
					$sql="SELECT Avatar, Name, Student_Id FROM Students WHERE Email='$_SESSION[mail]'";
					$mail = $_SESSION["mail"];
					}
					else{
					$mail = $_GET['new'];
					$sql="SELECT Avatar, Name, Student_Id FROM Students WHERE Email='$mail'";	
					}
					$query=mysql_query($sql,$con); 						// Ö´ÐÐSQLÖ÷¾ä
					$rows=mysql_fetch_array($query);
					$_SESSION["id"] = $rows[2];
					$_SESSION["friend"] = $mail;
					$sql = "SELECT * FROM Friends WHERE F_ID ='$_SESSION[id]'";
...
<div class="content">
 ...
 <a href="<?php echo "ly_pwd.php?news=".$mail?>"><?php echo $rows[1]; ?></a>
</br>
<?php
if($check!=1 && $mail!=$_SESSION["user"]){
?>
<form method="post" action="add_friend.php" onSubmit="return check()">
  <input type="submit" name="Submit" class="submit" value="Add Friend" />
  <br/>
    <br/>
</form>
<?php
...
<div class="row" >
  ...
  <?php
      if($check!=1 && $mail != $_SESSION["user"]){
    $sql="SELECT * FROM PersonalNews,Students WHERE PersonalNews.Email=Students.Email AND Students.Email = '$mail' order by News_Id desc limit 0,10";
    $query=mysql_query($sql,$con);            // Ö´ÐÐSQLÖ÷¾ä
    ?>
    ...
    if($_POST["$a"]) {
      ...
      mysql_query($sql_a, $con);
            ...
            $CommentMail = $rows["Email"];
            $sql_c = "INSERT INTO PersonalComments (Email, Time, Content, Type, No, Personal_Email) VALUES ('$user', '$time', '$content', '$b', '$a', '$CommentMail')";
  ...
  <?php 
    ...
    echo "The whole personal news can only be seen by friends";
  }
      if($check==1 || $mail == $_SESSION["user"]){
  ...
  <a name="<?php echo $c ?>"></a>
    ...
    if($_POST["$a"]) {
      ...
      mysql_query($sql_a, $con);
            if($rows["Email"]!=$_SESSION["user"]){
            $CommentMail = $rows["Email"];
            $sql_c = "INSERT INTO PersonalComments (Email, Time, Content, Type, No, Personal_Email) VALUES ('$user', '$time', '$content', '$b', '$dsb', '$CommentMail')";
            mysql_query($sql_c, $con);
        }
    }
    ?>
