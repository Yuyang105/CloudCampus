	...
	<div style="position:fixed; width:100%;height:50px; z-index:1; overflow-x:visible;">
      <div class="navbar navbar-default" role="navigation" >
        <div class="navbar-header">
          ...
          <a class="" href="personal_index.php"><span class="navbar-brand"><span class="fa fa-fw fa-cloud"></span> Cloud Campus</span></a>
          </div>
...
<li>   <ul class="acc-menu nav nav-list collapse in">
    ...
    <p id="back-to-top"><a href="#top"><span></span>Back to Top</a></p>  
        ...
        <div class="header">
            <h1 class="page-title">Personal News</h1>
                <ul class="breadcrumb">
	...
	<table width="100%">
		<tr>
			<td align="right">
				<input type="button" name="Post" value="Post News" onClick ="location.href='post_news.php'" class="btn btn-primary"/>
			</td>
		</tr>
		<tr><td>&nbsp;</td></tr>
	</table>
...
$con = mysql_connect("tcss545-aws-db.ce2yjmz6wbyv.us-west-2.rds.amazonaws.com","Yuyang","3350335012");
        ...
        mysql_select_db("CloudCampusOnAWS");  							//ѡ����ݿ� 
        if ($_SESSION["admin"]) {
        	$sql="SELECT * FROM PersonalNews,Students WHERE PersonalNews.Email=Students.Email ORDER BY News_Id desc"; // ��ѯ����ݿ��
        }
        else {
			$sql="SELECT * FROM FriendNews,Students WHERE FriendNews.Email=Students.Email ORDER BY News_Id desc"; // ��ѯ����ݿ��
		}
 		$query=mysql_query($sql,$con); 						//ִ��SQL���
 		$nums=mysql_num_rows($query); 						//����Ŀ��
 		$each_disNums=10;  									//ÿҳ��ʾ����Ŀ�� 
 		$sub_pages=2; 										//�� $subPage_type Ϊ 2 ʱ��ÿ����ʾ��ҳ��
 		$pageNums = ceil($nums/$each_disNums); 				//��ҳ��
 		$subPage_link="news.php?&page="; 					//ÿ���ҳ�����
    	...
    	$query_a=mysql_query($sql_a,$con); 						// ִ��SQL���
		...
		$sql_comment="SELECT * FROM Comments WHERE No='$a' AND Type='$b' order by Comment_Id";
 		$query_comment=mysql_query($sql_comment,$con); 						// ִ��SQL���
 		
		while($rows_comment=mysql_fetch_array($query_comment)){	
			$email=$rows_comment["Email"];
			$s="SELECT Avatar, Name FROM Students WHERE Email='$email'"; //SQL��䣬�˴���ҳ����
 			$q=mysql_query($s,$con);
