...
<?php
	$sql="SELECT * FROM FriendRequest WHERE Email='$_SESSION[user]' ORDER BY Request_Id desc";
	$query=mysql_query($sql,$con); 						//Ö´ÐÐSQLÓï¾ä
 	...
 	$subPage_link="request.php?&page="; 					//Ã¿¸ö·ÖÒ³µÄÁ´½Ó
	...
	$sql_request="SELECT * FROM FriendRequest WHERE Email='$_SESSION[user]' ORDER BY Request_Id desc limit $currentNums,$each_disNums";
	$query_request=mysql_query($sql_request,$con); 
	
	while($rows=mysql_fetch_array($query_request)){
		$sql1="SELECT Avatar, Name,Email FROM Students WHERE Student_Id='$rows[Req_ID]'";
		$query1=mysql_query($sql1,$con); 						// Ö´ÐÐSQLÖ÷¾ä
		$rows1=mysql_fetch_array($query1);	
?>
<div style="border: 2px #CCCCCC solid; width:90%; margin: 0 auto;">
    ...
    <th width="15">
        ...
        <td width="3%">
    		<td align="left" bgcolor="#FFFFFF">
    			<a href="<?php echo "page.php?new=".$rows1[2]?>"><?php echo '<span style="font-weight:bolder;">'.$rows1[1].'</span>'?></a>
    		</td>
            </td>
            </br>
...
<tr>
<th width="3%">
  <td align="left" width="50%"> <font face="Verdana">has sent you a friend request</font></td>
